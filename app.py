...
icon_gear = (
    '<circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0'
    ...
    " 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0"
    " 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0"
    " 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0"
    " 0 1 0 2.83l-.06-.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0"
    ' 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"></path>'
)
icon_db = (
    '<ellipse cx="12" cy="5" rx="9" ry="3"></ellipse><path d="M21 12c0'
    " 1.66-4 3-9 3s-9-1.34-9-3\"></path><path d=\"M3 5v14c0 1.66 4 3 9 3s9-1.34"
    ' 9-3V5"></path>'
)
icon_bolt = (
...
icon_doc = (
    ...
    ' 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line'
    ' x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8"'
    ' y2="17"></line><polyline points="10 9 9 9 8 9"></polyline>'
)
...
with st.sidebar:
  ...
  # Jika belum login, tampilkan form input nama & email sederhana di sidebar
  if not st.session_state.logged_in:
    with st.form("form_login"):
      st.write("Silakan isi identitas Anda untuk mulai menggunakan aplikasi:")
      ...
      if btn_masuk:
        if input_nama and input_email:
          ...
          st.warning("Mohon isi Nama dan Email terlebih dahulu.")
    st.stop()  # Hentikan eksekusi sebelum login berhasil
  else:
    # Jika sudah login, tampilkan nama & email mereka di sidebar
...
with st.form("form_valuasi_properti"):
  ...
  with col1:
    luas_tanah = st.number_input(
        "Luas Tanah (m²)", min_value=10, max_value=5000, value=120, step=10, key="lt"
    )
    ...
    jumlah_kamar = st.number_input(
        "Jumlah Kamar Tidur", min_value=1, max_value=20, value=3, step=1, key="jk"
    )
    jumlah_lantai = st.number_input(
        "Jumlah Lantai", min_value=1, max_value=5, value=2, step=1, key="jl"
    )

  with col2:
    jumlah_garasi = st.number_input(
        "Kapasitas Garasi/Carport",
        min_value=0,
        max_value=10,
        value=1,
        step=1,
    ...
    usia_bangunan = st.number_input(
        "Usia Bangunan (Tahun)", min_value=0, max_value=50, value=2, step=1, key="ub"
    )
    jarak_ke_tol = st.number_input(
        "Jarak ke Pintu Tol (km)",
        min_value=0.0,
        max_value=50.0,
        value=3.0,
        step=0.1,
  ...
  with col_sub1:
    kondisi = st.selectbox(
        ...
        ["Siap Huni", "Baru Renovasi", "Perlu Renovasi"],
        key="kondisi",
    )
  with col_sub2:
...
if submit_btn:
  ...
  try:
    ...
    with col_res1:
      ...
      st.success(
          f"▪ Kalkulasi untuk **{st.session_state.user_name}** berhasil"
          " dieksekusi."
      )
    with col_res2:
